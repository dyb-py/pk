- 网易云音乐

~~~markdown
网易云音乐的评论请求，涉及到了JS加密
JS加密：
		用户在发送请求的时候，请求的参数由JS按照一定规则进行加密。
		解决方法：
			1. 按照一定规则进行解密（需要有精通JS的能力）
JS混淆：
		如家：350/晚
		烫烫烫烫烫烫锟斤拷锟斤拷锟斤拷/晚---->JS处理---->350/晚
网易云音乐：JS加密
爬虫：
		以数据为导向
		获得数据的主要手段（核心）：请求和响应
~~~

### Selenium

~~~markdown
	Selenium是一个web用的自动化的测试工具，最开始是用于自动化运维测试的。类似于，按键精灵。无界面浏览器可以根据自动化指令，对浏览器进行操作。实际上Selenium支持市面上绝大多数的浏览器。
	使用Selenium，就可以让浏览器根据我们的指令去：加载网页、获取页面、页面元素获取、页面截屏、对页面进行一些操作。
	Selenium本身，是一个库，Selenium本身是不带浏览器。Selenium实际上是操控了本地安装的浏览器。
	无头浏览器/无界面浏览器：Selenium本身操控的浏览器是和完整的浏览器有一定区别的。
~~~

#### 快速入门

~~~python
# 导包：webdriver，网页驱动
from selenium import webdriver
import time
import jieba
import wordcloud

# 创建一个driver对象，声明该对象使用的是谷歌浏览器的驱动
driver = webdriver.Chrome(executable_path=r'E:\Python186共享文件夹\第三阶段\代码\day10\chromedriver.exe')
# 相当于在浏览器的地址栏输入"http://www.baidu.com"，并且将浏览器页面进行展示
driver.get('https://music.163.com/#/song?id=561493928')
# 对driver对象进行操作
# driver.switch_to_frame('g_iframe')
# 切换当前页面的iframe，iframe：html里面可以嵌套一个html，数据可能在另一个html中，所以需要切换
driver.switch_to.frame('g_iframe')
# 可以选择当前页面的元素：选择方式有：
    # xpath、classname、CSS、id等等
    # 获得到的是selenium的webdelement对象，可以对该对象调用text方法，显示数据
while True:
    # 需要等待一段时间，让页面加载完毕，再对页面进行解析
    time.sleep(3)
    # 执行JS代码，让页面向下滑动1000个单位
    driver.execute_script('var q=document.documentElement.scrollTpo=1000')
    # 使用xpath选择器，获取评论的元素
    data = driver.find_elements_by_xpath(
        '/html/body/div[3]/div[1]/div/div/div[2]/div/div[2]/div[2]//div/div[2]/div[1]/div')
    print(data)
    commont = ''
    for i in data:
        commont_str = i.text
        s = jieba.cut(commont_str.split('：')[1])
        for j in s:
            commont += j+','
    print(commont)
    with open('wyyyy.txt','a',encoding='utf-8') as w:
        w.write(commont)
    # 点击事件
    # 使用link_text选择器获取到下一页按钮的元素
    b = driver.find_element_by_link_text('下一页')

    print(b.text)
    # 运行js代码，点击下一页，获得下一页数据。
    driver.execute_script("arguments[0].click();", b)


~~~

- 作业

~~~markdown
要求使用百度，做如下事情：
	1. 在无界面浏览器中访问百度
	2. 在搜索框中输入续馥霖，并且点击立即百度
	3. 在结果页面中，点击百度图片
	4. 保存下载续馥霖的前五张图片
~~~

