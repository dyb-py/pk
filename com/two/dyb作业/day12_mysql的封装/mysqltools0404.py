import MySQLdb
import com.two.dyb作业.day12_mysql的封装.settings as s
# version4.0
class MySqlTools(object):

    # 初始化方法
    def __init__(self,*,db,user,password,host='localhost',port=3306,charset='utf8s'):
        '''
        初始化方法
    ...
    def get_tool(cls):
        ...
        data={
            'db' : s.data.setdefault('db','user'),
            'user' : s.data.setdefault('user','root'),
            'password' : s.data.setdefault('password','123456'),
            'host' : s.data.setdefault('host','localhost'),
            'port' : s.data.setdefault('port',3306),
            'charset' : s.data.setdefault('charset','utf8')}
