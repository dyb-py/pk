...
class MySqlTools(object):
    ...
    @classmethod
    def get_tool(cls,*,db,user,password,host,port,charset):
        try:
            conn=MySQLdb.connect(db=db,user=user,password=password,
            ...
            # 创建工具对象
            conn.close()
            return cls(db=db,user=user,password=password,
                       host=host,port=port,charset=charset)
...
if __name__ == '__main__':
    ...
    # del tool
    tool=MySqlTools.get_tool(db='test123',
                             user='root',
                             password='123456',
                             host='localhost',
                             port=3306,
                             charset='utf8')
    print(tool.get('student', ['*']))