...
class ZhuCe:
    ...
    def __init__(self):
        ...
        self.user = User(self.ids, self.name, self.pwd, self.phone, self.mail)
...
class InputBook():
    #输入书籍信息
    def __init__(self):
        self.ids = int(input('输入ids'))
        self.name = input('输入name')
        self.author = input('输入作者')
        self.price = float(input('输入价格'))
        self.category = input('输入类别')
        self.desc = input('输入评价')
        self.book=Book(self.ids,self.name,self.author,self.price,self.category,self.desc)
    def getBook(self):
        return self.book
